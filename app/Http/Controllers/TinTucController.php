...
class TinTucController extends Controller
{
    public function index(Request $request)
    {
        ...
        $lstTintuc = $query->with('nhanvien')->orderBy('created_at', 'desc')->paginate(10);


        $title = "Quản lý tin tức - EbookCare";
        //$tintuc = TinTuc::where('trangthai', 1)->orderBy('ngaydang', 'desc')->get();
        return view('admin.tintuc.index', compact('lstTintuc', 'title'));
    }

    public function xemTinTuc($id)
    {
        $baiviet = TinTuc::where('id', $id)->firstOrFail();
        $title = $baiviet->tieude;

        return view('admin.tintuc.detail', compact('baiviet', 'title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $ls->save();
        return redirect()->route('tintuc.index')->with('success', 'Tin tức đã được đăng!');
    }
    ...
    public function edit(string $id)
    {
        ...
        if ($user->role == 'admin' || ($nhanvien && $tintuc->id_nhanvien == $nhanvien->id)) {
            return view('admin.tintuc.edit', compact('title', 'tintuc'));
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $ls->save();
        return redirect()->route('tintuc.index')->with('success', 'Cập nhật dữ liệu thành công!');
    }
    ...
    public function updateStatus(Request $request)
    {
        ...
        if (!$tintuc) {
            return response()->json(['success' => false, 'message' => 'Bản tin không tồn tại!']);
        }
        ...
        $user = Auth::user();
        $ls = new LichSu();
        $ls->noidung = 'Người dùng ' . $user->name . ' (' . $user->email . ') đã cập nhật trạng thái "' . $trangthai . '" cho Tin Tức "'  . $tintuc->tieude . '" vào CSDL!';
        $ls->save();
    ...
    public function delete($id)
    {
        ...
        if ($user->role == 'admin' || ($nhanvien && $tintuc->id_nhanvien == $nhanvien->id)) {
            ...
            $ls->save();
            return redirect()->route('tintuc.index')->with('success', 'Xóa dữ liệu thành công!');
        } else {
            return redirect()->route('tintuc.index')->with('error', 'Chỉ admin mới được xóa bài viết.');
        }
    }


    //Trang chủ
    public function showTrangTin($loai)
    {
        $tintucs = TinTuc::where('trangthai', 1)
            ->when(isset($loai), function ($query) use ($loai) {
                return $query->where('loai', $loai);
            })
            ->with('nhanvien') // Thêm quan hệ để lấy thông tin nhân viên
            ->orderBy('created_at', 'desc')
            ->paginate(12); // Phân trang


        $title = 'Trang tin - Cập nhật thông tin quan trọng về sức khỏe ';
        return view('trangchu.trangtin.index', compact('tintucs', 'title'));
    }

    public function detail($loai, $id)
    {
        $baiviet = TinTuc::where('id', $id)->where('trangthai', 1)->firstOrFail();
        $title = $baiviet->tieude;

        // Lấy 4 tin mới nhất khác với bài viết hiện tại
        $relatedNews = TinTuc::where('id', '!=', $id)
            ...
            ->take(5)
            ->get();
        return view('trangchu.trangtin.detail', compact('baiviet', 'title', 'relatedNews'));
    }
}
